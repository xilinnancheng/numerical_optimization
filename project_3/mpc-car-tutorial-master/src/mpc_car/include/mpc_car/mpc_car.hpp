...
namespace mpc_car
{
  ...
  public:
    ...
    static inline double objectiveFuncALM(void *ptrObj,
                                          const double *x,
                                          double *grad,
                                          const int n)
    {
      ...
      for (int i = 0; i < obj.N_; ++i)
      {
        ...
        double delta = inputs.col(i)(1);
        double grad_a, grad_delta;
        total_cost += obj.box_constrant_ALM(a, -obj.a_max_, obj.a_max_, obj.alm_rho_, obj.alm_mu_[i], grad_a);
        grad_inputs.col(i)(0) += grad_a;
        total_cost += obj.box_constrant_ALM(delta, -obj.delta_max_, obj.delta_max_, obj.alm_rho_, obj.alm_mu_[obj.N_ + i], grad_delta);
        grad_inputs.col(i)(1) += grad_delta;
      }
      for (int i = 0; i < obj.N_ - 1; ++i)
      {
        ...
        double ddelta = delta_k_1 - delta_k;
        double grad_ddelta;
        total_cost += obj.box_constrant_ALM(ddelta,
                                            -obj.ddelta_max_ * obj.dt_,
                                            obj.ddelta_max_ * obj.dt_,
                                            obj.alm_rho_, obj.alm_mu_[m * obj.N_ + i],
                                            grad_ddelta);
        grad_inputs.col(i)(1) -= grad_ddelta;
        grad_inputs.col(i + 1)(1) += grad_ddelta;
      }
    ...
    double box_constrant_ALM(const double &x,
                             const double &l,
                             const double &u,
                             const double &rho,
                             const double &mu,
    ...
    {
      double coffe = mu / rho;
      double l_violation = l - x + coffe;
      double u_violation = x - u + coffe;
      if (l_violation > 0)
      {
        grad = -rho * l_violation;
        return 0.5 * rho * l_violation * l_violation;
      }
      else if (u_violation > 0)
      {
        grad = rho * u_violation;
        return 0.5 * rho * u_violation * u_violation;
      }
      else
      {
        grad = 0;
    ...
    int solveNMPCALM(const VectorX &x0_observe)
    {
      historyInput_.pop_front();
      historyInput_.push_back(predictInput_.front());
      // x0_observe_ = x0_observe;
      ...
      alm_gamma_ = 1.0;
      alm_belta_ = 1000;
      double epsilon_prec = 1e-3;
      double epsilon_cond = 1e-3;
      int ret;
      int iteration_num = 0;

      alm_mu_.resize((m + 1) * N_);
      alm_mu_.setZero();
      double violation_infinity_norm = 1e-6;
      double minObjective;
      while (1)
      {
        lbfgs::lbfgs_parameter_t lbfgs_params;
        ...
        lbfgs_params.g_epsilon = 0.0;
        lbfgs_params.min_step = 1e-32;
        lbfgs_params.delta = 1e-4; //std::pow(alm_xi_, iteration_num) * std::min<double>(1, violation_infinity_norm);
        lbfgs_params.line_search_type = 0;
        ret = lbfgs::lbfgs_optimize(m * N_, x, &minObjective, &objectiveFuncALM, nullptr, nullptr, this, &lbfgs_params);

        Eigen::VectorXd violation_vec((m + 1) * N_);
        violation_vec.setZero();
        violation_infinity_norm = -1e10;
        for (int i = 0; i < N_; ++i)
        {
          double a = inputs.col(i)(0);
          double delta = inputs.col(i)(1);
          double grad_a, grad_delta;
          double a_violation = std::sqrt(2.0 / alm_rho_ * box_constrant_ALM(a, -a_max_, a_max_, alm_rho_, alm_mu_[i], grad_a));
          double delta_violation = std::sqrt(2.0 / alm_rho_ * box_constrant_ALM(delta, -delta_max_, delta_max_, alm_rho_, alm_mu_[N_ + i], grad_delta));
          violation_infinity_norm = a_violation > violation_infinity_norm ? a_violation : violation_infinity_norm;
          violation_infinity_norm = delta_violation > violation_infinity_norm ? delta_violation : violation_infinity_norm;
          if (a_violation > 0.0)
          {
            a_violation -= alm_mu_[i] / alm_rho_;
          }

          if (delta_violation > 0.0)
          {
            delta_violation -= alm_mu_[N_ + i] / alm_rho_;
          }
          violation_vec[i] = a_violation;
          violation_vec[N_ + i] = delta_violation;
        }
        ...
        for (int i = 0; i < N_ - 1; ++i)
        {
          ...
          double ddelta = delta_k_1 - delta_k;
          double grad_ddelta;
          double ddelta_violation = std::sqrt(2.0 / alm_rho_ * box_constrant_ALM(ddelta, -ddelta_max_ * dt_, ddelta_max_ * dt_, alm_rho_, alm_mu_[m * N_ + i], grad_ddelta));
          violation_infinity_norm = ddelta_violation > violation_infinity_norm ? ddelta_violation : violation_infinity_norm;
          if (ddelta_violation > 0.0)
          {
            ddelta_violation -= alm_mu_[m * N_ + i] / alm_rho_;
          }
          violation_vec[m * N_ + i] = ddelta_violation;
        }
        ...
        for (int i = 0; i < (m + 1) * N_; ++i)
        {
          alm_mu_[i] = std::max(alm_mu_[i] + alm_rho_ * violation_vec[i], 0.0);
        }

        alm_rho_ = std::min((1 + alm_gamma_) * alm_rho_, alm_belta_);
        ++iteration_num;
      }
